...
class RBM(object):
    ...
    name : str
        Name of the model
    W : array-like, shape (n_hidden, n_visible)
        Weight matrix
    v_bias : array-like, shape (n_visible,)
        Visible bias vector
    h_bias : array-like, shape (n_hidden,)
        Hidden bias vector
    k : int
        Number of Gibbs sampling steps
    # energy_type : str
    ...
    energy_type: str = 'hopfield' # vestigial, not doing anything
    optimizer: str = 'SGD'
        ...
        while self.epoch < self.max_epochs:
            ...
            for _, v_data in enumerate(train_data):
                ...
                if self.train_algo == 'PCD':
                    ...
                    print(
                        'Warning: No physical measurements are implemented for RDM training algorithm. Use hRDM or vRDM instead.'
                    )
    ...
    def after_step_keepup(self):
        # self.W_t = self.W.t() ## done in clip_weights
        self.clip_weights()
        self.clip_bias()
